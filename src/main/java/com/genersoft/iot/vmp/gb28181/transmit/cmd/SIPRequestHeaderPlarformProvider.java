...
public class SIPRequestHeaderPlarformProvider {
    ...
    public Request createRegisterRequest(@NotNull Platform parentPlatform, long CSeq, String fromTag, String toTag, CallIdHeader callIdHeader, int expires) throws ParseException, InvalidArgumentException, PeerUnavailableException {
        ...
        //请求行
        SipURI requestLine = SipFactory.getInstance().createAddressFactory()
                .createSipURI(parentPlatform.getServerGBId(),
        ...
        ViaHeader viaHeader = SipFactory.getInstance().createHeaderFactory()
                .createViaHeader(parentPlatform.getDeviceIp(),
        ...
        //from
        SipURI fromSipURI = SipFactory.getInstance().createAddressFactory()
                .createSipURI(parentPlatform.getDeviceGBId(), sipConfig.getDomain());
        Address fromAddress = SipFactory.getInstance().createAddressFactory()
                .createAddress(fromSipURI);
        FromHeader fromHeader = SipFactory.getInstance().createHeaderFactory()
                .createFromHeader(fromAddress, fromTag);
        //to
        SipURI toSipURI = SipFactory.getInstance().createAddressFactory()
                .createSipURI(parentPlatform.getDeviceGBId(), sipConfig.getDomain());
        ...
        request = SipFactory.getInstance()
                .createMessageFactory()
                .createRequest(requestLine,
                        Request.REGISTER,
                        callIdHeader,
                        cSeqHeader,
                        fromHeader,
        ...
        SipURI sipURI = SipFactory.getInstance()
                .createAddressFactory()
                .createSipURI(parentPlatform.getDeviceGBId(), sipAddress);

        Address concatAddress = SipFactory.getInstance()
                .createAddressFactory()
        ...
        ExpiresHeader expiresHeader = SipFactory.getInstance()
                .createHeaderFactory().createExpiresHeader(expires);
